...
namespace YnabReportsAPI.Transactions.Services
{
    ...
    public class TransactionService : ITransactionService
    {
        ...
        private IEnumerable<Transaction> GetSingleTransactions(IEnumerable<YnabTransaction> ynabTransactions)
            => ynabTransactions
            .Where(x => !x.Deleted && x.Approved && !x.SubTransactions.Any())
            .Select(x => new Transaction(x.Date, this.ConvertMilliUnitsToDecimal(x.Amount), x.CategoryName));

        private IEnumerable<Transaction> GetSplitTransactions(IEnumerable<YnabTransaction> ynabTransactions)
        {
            var transactions = new List<Transaction>();

            IEnumerable<YnabTransaction>? splitTransactions = ynabTransactions
                .Where(x => !x.Deleted && x.Approved && x.SubTransactions.Any());

            foreach (var splitTransaction in splitTransactions)
            {
                transactions.AddRange(
                    splitTransaction.SubTransactions
                    .Where(x => !x.Deleted)
                    .Select(x => new Transaction(splitTransaction.Date, this.ConvertMilliUnitsToDecimal(x.Amount), x.CategoryName)));
            }
